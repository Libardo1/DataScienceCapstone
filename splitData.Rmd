...
repeat {
    ...
    if (length(cur_chunk) > 0) {
        ...
        if (length(cur_chunk) == 0) {
            ...
            curTDM <- removeSparseTerms(TermDocumentMatrix(curLineCorpus),
                                        chunkSparsity)
            ...
            if (firstChunk == TRUE) {
                ...
                termFreqs <- termFreqs[curChunkTermFreqs]
                termFreqs <- as.data.frame(termFreqs)
                
                termFreqs$count[is.na(termFreqs$count)] = 0
                termFreqs$count[is.na(termFreqs$i.count)] = 0
                
                termFreqs$count <- termFreqs$count + termFreqs$i.count
