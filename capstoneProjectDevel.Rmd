...
```{r languageDetection}
textDocumentLanguages <- c("english",
                           "finnish",
                           "german",
                           "russian-iso8859_5",
                           "russian-koi8_r",
...
profileDb <- TC_byte_profiles[names(TC_byte_profiles) %in% 
                              textDocumentLanguages]

lines_to_read <- 1000
inputTextFilePath <- "./OutputData//en_US/en_US.blogs1p00Percent.txt"
...
validLanguageIdx <- which(!grepl(paste0(textFileLanguage,"[a-z0-9_]*"),
                                curChunkLanguage))
```
...
eightySevenPercentIdx <- which(termCDF >= 0.87)[1]
...
text(sixtyPercentIdx,0.4,sprintf("# of words: %d", fiftyPercentIdx),pos=4,
     col="firebrick")
...
lines(seq(from=1,to=ninetyPercentIdx,by=1),0.9*rep(1,ninetyPercentIdx),
      col="springgreen4",lty=2,lwd=2)
arrows(eightySevenPercentIdx,0.72,ninetyPercentIdx,0.9,
       lwd=2,length=0.15,col="springgreen4")
text(eightySevenPercentIdx,0.72,sprintf("# of words: %d", ninetyPercentIdx),
...
cumualtiveSum <- cumsum(termFreqs)
