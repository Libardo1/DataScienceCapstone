...
predictorVocabulary <- character()
for (n in seq_len(length(triTdm))) {
    if (n %% 100 == 0) {
        print(sprintf("%d (Out of %d)", n, length(triTdm)))
    }
    curWord <- unlist(str_split(names(triTdm[n])," "))

    predictorVocabulary <- append(predictorVocabulary,
                                  curWord[!curWord %in% predictorVocabulary])    
}
predictorVocabulary <- sort(predictorVocabulary)
